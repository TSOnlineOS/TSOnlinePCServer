...
namespace TS_Server.Client
{
    public class TSCharacter
    {
        ...
        public void loadCharDB()
        {
            ...
            charId = 1;// data.GetInt32("id");
            level = 200;//data.GetByte("level");
            hp = 100000;//data.GetInt32("hp");
            hp = Math.Max(1, hp); // 0 HP ออกจากระบบถ้าตายในการสู้รบ
            sp = 1;//data.GetInt32("sp");
            mag = 1;//data.GetInt32("mag");
            atk = 10000;//data.GetInt32("atk");
            def = 10000;//data.GetInt32("def");
            hpx = 10000;//data.GetInt32("hpx");
            spx = 1;//data.GetInt32("spx");
            agi = 10;//data.GetInt32("agi");

            hp2 = 1;//data.GetInt32("hp2");
            sp2 = 1;//data.GetInt32("sp2");
            mag2 = 1;//data.GetInt32("mag2");
            atk2 = 1;//data.GetInt32("atk2");
            def2 = 1;//data.GetInt32("def2");
            agi2 = 10;//data.GetInt32("agi2");
            skill_point = 300;//data.GetInt32("sk_point");
            stt_point = 1000;//data.GetInt32("stt_point");

            sex = 1;//data.GetByte("sex");
            ghost = 0;//data.GetByte("ghost");
            god = 0;//data.GetByte("god");
            style = 1;//data.GetByte("style");
            hair = 1;//data.GetByte("hair");
            face = 1;//data.GetByte("face");

            color1 = 100;//data.GetUInt32("color1");
            color2 = 100;//data.GetUInt32("color2");
            mapID = 12000;//data.GetUInt16("map_id");
            mapX = 500;//data.GetUInt16("map_x");
            mapY = 100;//data.GetUInt16("map_y");

            currentxp = 1;//data.GetInt32("exp");
            totalxp = 1;//data.GetUInt32("exp_tot");
            honor = 100;//data.GetUInt32("honor");

            element = 1;//data.GetByte("element");
            rb = 2;//data.GetByte("reborn");
            job = 2;//data.GetByte("job");

            gold = 99999999;//data.GetUInt32("gold");
            gold_bank = 99999999;//data.GetUInt32("gold_bank");


            //name = data.GetString("name");
            name = Encoding.ASCII.GetBytes("Homer");//(byte[])(data["name"]);

            var equip_data = Encoding.ASCII.GetBytes("");// (byte[])data["equip"];
            loadEquipment(equip_data);

            var inventory_data = Encoding.ASCII.GetBytes("");//(byte[])data["inventory"];
            inventory.loadContainer(inventory_data);

            var storage_data = Encoding.ASCII.GetBytes("");//(byte[])data["storage"];
            storage.loadContainer(storage_data);

            var bag_data = Encoding.ASCII.GetBytes("");//(byte[])data["bag"];
            bag.loadContainer(bag_data);
            var skill_data = Encoding.ASCII.GetBytes("");//(byte[])data["skill"];
            loadSkill(skill_data);

            var hotkey_data = Encoding.ASCII.GetBytes("");//(byte[])data["hotkey"];
            loadHotkey(hotkey_data);

            pet_battle = -1;// (sbyte)data.GetByte("pet_battle");
        ...
        public void loadPet()
        {
            ...
            //{
                ...
                //pet[0] = new TSPet(this, sid, (byte)s);
                ///pet[0].loadPetDB();
            //}
            ///data.Close();
            //c.connection.Close();
        ...
        public void loadEquipment(byte[] data)
        {
            ...
            equipment[0] = new TSEquipment(null, 15038, 1, 1);
            equipment[0].equip.duration = 0;// data[pos + 3];
            equipment[0].equip.elem_type = 3;// data[pos + 4];
            equipment[0].equip.elem_val = 100;// data[pos + 5] + (data[pos + 6] << 8);
            equipment[0].char_owner = this;
            nb_equips++;
            addEquipBonus(ItemData.itemList[15038].prop1, ItemData.itemList[15038].prop1_val, 0);
            addEquipBonus(ItemData.itemList[15038].prop2, ItemData.itemList[15038].prop2_val, 0);
        }
